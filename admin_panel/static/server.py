...
SERVER_CHECK_INTERVAL_SECONDS = 3 * 60  # هر ۳ دقیقه کانفیگ‌های لینک ساب مادر چک شوند
...
async def _server_status_loop():
    # یک بار آفلاین دیدن تست لزوماً یعنی سرور واقعاً قطعه؛ ممکنه جیتر لحظه‌ای
    # شبکه باشه. پس فقط وقتی یه کانفیگ دو دور متوالی (حدود ۶ دقیقه) پشت‌سرهم
    # آفلاین دیده بشه پوش قطعی می‌فرستیم؛ به‌محضی که یه دور آنلاین ببینیم
    # استریک ریست می‌شه.
