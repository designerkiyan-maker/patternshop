        ...
        <div class="gl-row-wrap bn-card-anim" style="animation-delay:${Math.min(i * 30, 260)}ms">
          <div class="gl-row">
            ...
            <div class="gl-row-main">
              <span class="gl-row-title">${esc(o.product_name)}</span>
              <span class="gl-row-sub">${esc(o.username || o.user_id)} · ${fmtDate(o.created_at)}</span>
            </div>
            <div class="gl-row-trail">
              <span class="gl-row-amount mono">${fmt(o.final_price ?? o.base_price)} ت</span>
              ${o.receipt_file_id ? `<button class="gl-btn gl-btn-ghost" data-receipt="order:${o.id}">رسید</button>` : ''}
              ${historyBtn('order', o.id)}
            </div>
          </div>
          ${canAct && ordersStatus === 'pending' ? `<div class="gl-row-actions">
            <button class="gl-btn gl-btn-ok" data-approve="${o.id}">تایید</button>
            <button class="gl-btn gl-btn-no" data-reject="${o.id}">رد</button>
          </div>` : ''}
        </div>
      `).join('') || `<div class="empty-state"><div class="icon">${svg('empty')}</div>سفارشی در این وضعیت نیست</div>`}
    </div>
  `);
  $$('.gl-seg-btn', content()).forEach(b => b.addEventListener('click', () => { ordersStatus = b.dataset.status; renderOrders(); }));
  $$('[data-receipt]', content()).forEach(b => b.addEventListener('click', () => {
    const [kind, id] = b.dataset.receipt.split(':');
    showReceiptModal(kind, id);
  }));
      ...
      ${topups.map((t, i) => `
        <div class="gl-row-wrap bn-card-anim" style="animation-delay:${Math.min(i * 30, 260)}ms">
          <div class="gl-row">
            ...
            <div class="gl-row-main">
              <span class="gl-row-title">${esc(t.username || t.user_id)}</span>
              <span class="gl-row-sub">${fmtDate(t.created_at)}</span>
            </div>
            <div class="gl-row-trail">
              <span class="gl-row-amount mono">${fmt(t.amount)} ت</span>
              ${t.receipt_file_id ? `<button class="gl-btn gl-btn-ghost" data-receipt="topup:${t.id}">رسید</button>` : ''}
            </div>
          </div>
          ${canAct && topupsStatus === 'pending' ? `<div class="gl-row-actions">
            <button class="gl-btn gl-btn-ok" data-approve="${t.id}">تایید</button>
            <button class="gl-btn gl-btn-no" data-reject="${t.id}">رد</button>
          </div>` : ''}
        </div>
      `).join('') || `<div class="empty-state"><div class="icon">${svg('empty')}</div>درخواستی در این وضعیت نیست</div>`}
    </div>
  `);
  $$('.gl-seg-btn', content()).forEach(b => b.addEventListener('click', () => { topupsStatus = b.dataset.status; renderTopups(); }));
  $$('[data-receipt]', content()).forEach(b => b.addEventListener('click', () => {
    const [kind, id] = b.dataset.receipt.split(':');
    showReceiptModal(kind, id);
  }));
...
/* ---------------------------------------------------------- users: glass */
function renderUsersGlass(res, pages) {
  setContent(`
    <div class="gl-hero">
      <div><h2>کاربران</h2><p>${fmt(res.total)} کاربر ثبت‌شده</p></div>
      <div class="gl-seg">${['all', 'active', 'expired', 'blocked'].map(s => `<button class="gl-seg-btn ${s === usersState.status ? 'active' : ''}" data-ustatus="${s}">${USERS_STATUS_LABEL[s]}</button>`).join('')}</div>
    </div>
    <input class="gl-search" id="user-search" placeholder="جستجو: آیدی، یوزرنیم، نام..." value="${esc(usersState.q)}" style="margin-bottom:14px">
    <div class="gl-list">
      ${res.items.map((u, i) => `
        <div class="gl-row bn-card-anim" style="animation-delay:${Math.min(i * 25, 240)}ms">
          ${glAvatar((u.first_name || u.username || '؟').trim().charAt(0).toUpperCase(), i)}
          <div class="gl-row-main">
            <span class="gl-row-title">${esc(u.username ? '@' + u.username : (u.first_name || '—'))}</span>
            <span class="gl-row-sub">ID: ${u.telegram_id} · عضویت ${fmtDate(u.joined_at)}</span>
          </div>
          <div class="gl-row-trail">
            ${glPill(u.is_blocked ? 'مسدود' : 'فعال', u.is_blocked ? 'no' : 'ok')}
            <button class="gl-btn gl-btn-ghost" data-detail="${u.telegram_id}">جزئیات</button>
            ${hasPerm('users') ? (u.is_blocked
              ? `<button class="gl-btn gl-btn-ok" data-unblock="${u.telegram_id}">رفع مسدودی</button>`
              : `<button class="gl-btn gl-btn-no" data-block="${u.telegram_id}">مسدودسازی</button>`) : ''}
          </div>
        </div>
      `).join('') || `<div class="empty-state"><div class="icon">${svg('empty')}</div>کاربری یافت نشد</div>`}
    </div>
...
/* ------------------------------------------------------- catalog: glass -- */
function renderCatalogGlass(categories, products) {
  setContent(`
    <div class="gl-hero">
      <div><h2>ویترین محصولات</h2><p>${fmt(products.length)} محصول در ${fmt(categories.length)} دسته‌بندی</p></div>
      <div class="gl-seg">
        <button class="gl-seg-btn ${catalogTab === 'products' ? 'active' : ''}" data-t="products">محصولات</button>
        <button class="gl-seg-btn ${catalogTab === 'categories' ? 'active' : ''}" data-t="categories">دسته‌بندی‌ها</button>
      </div>
    </div>
    <div id="catalog-body"></div>
  `);
  $$('.gl-seg-btn[data-t]', content()).forEach(b => b.addEventListener('click', () => { catalogTab = b.dataset.t; renderCatalog(); }));

  const body = $('#catalog-body');
  if (catalogTab === 'categories') {
    body.innerHTML = `
      <div class="gl-toolbar"><button class="gl-btn gl-btn-ok" id="add-cat">+ دسته‌بندی جدید</button></div>
      <div class="gl-list">
        ${categories.map((c, i) => `
          <div class="gl-row bn-card-anim" style="animation-delay:${Math.min(i * 25, 220)}ms">
            ${glAvatar(c.name.trim().charAt(0), i)}
            <div class="gl-row-main"><span class="gl-row-title">${esc(c.name)}</span></div>
            <div class="gl-row-trail">
              ${glPill(c.is_active ? 'فعال' : 'غیرفعال', c.is_active ? 'ok' : 'no')}
              <button class="gl-btn gl-btn-ghost" data-toggle-cat="${c.id}">${c.is_active ? 'غیرفعال کن' : 'فعال کن'}</button>
              <button class="gl-btn gl-btn-no" data-del-cat="${c.id}">حذف</button>
            </div>
          </div>
        `).join('') || `<div class="empty-state"><div class="icon">${svg('empty')}</div>دسته‌بندی‌ای نیست</div>`}
      </div>`;
  ...
  body.innerHTML = `
    <div class="gl-toolbar"><button class="gl-btn gl-btn-ok" id="add-prod">+ محصول جدید</button></div>
    <div class="gl-card-grid">
      ${products.map((p, i) => `
        <div class="gl-card bn-card-anim" style="animation-delay:${Math.min(i * 30, 260)}ms">
          <span class="gl-card-price mono" style="color:${GLASS_COLORS[i % GLASS_COLORS.length]}">${fmt(p.price)} ت</span>
          <span class="gl-card-title">${esc(p.name)}</span>
          <span class="gl-card-sub">${esc(p.category_name)} · موجودی: ${p.is_auto_provision ? 'خودکار' : fmt(p.stock)}</span>
          ${glPill(p.is_active ? 'فعال' : 'غیرفعال', p.is_active ? 'ok' : 'no')}
          <div class="gl-card-actions">
            ${!p.is_auto_provision ? `<button class="gl-btn gl-btn-ghost" data-configs="${p.id}">بانک کانفیگ</button>` : ''}
            <button class="gl-btn gl-btn-ghost" data-toggle-prod="${p.id}">${p.is_active ? 'غیرفعال' : 'فعال'}</button>
            <button class="gl-btn gl-btn-no" data-del-prod="${p.id}">حذف</button>
          </div>
        </div>
      `).join('') || `<div class="empty-state" style="grid-column:1/-1"><div class="icon">${svg('empty')}</div>محصولی نیست</div>`}
    </div>`;
...
function renderSupportGlass(convs) {
  setContent(`
    <div class="gl-hero"><div><h2>صندوق پشتیبانی</h2><p>${fmt(convs.length)} گفتگو</p></div></div>
    <div class="gl-list" id="support-inbox-list">${supportInboxRowsHtmlGlass(convs)}</div>
  `);
  $$('[data-open]', content()).forEach(b => b.addEventListener('click', () => showSupportChat(Number(b.dataset.open))));
  SUPPORT_POLL_TIMER = setInterval(async () => {
    if (CURRENT_TAB !== 'support') return stopSupportPoll();
...
function supportInboxRowsHtmlGlass(convs) {
  return convs.map((c, i) => `
    <div class="gl-row bn-card-anim" style="animation-delay:${Math.min(i * 25, 220)}ms">
      ${glAvatar((c.user_name || c.user_username || '؟').trim().charAt(0), i)}
      <div class="gl-row-main">
        <span class="gl-row-title">${esc(c.user_name || c.user_username || ('#' + c.user_id))}${c.locked_for_me ? ' 🔒' : ''}</span>
        <span class="gl-row-sub">${c.last_sender === 'admin' ? '↩ ' : ''}${esc(c.last_message || '')}</span>
      </div>
      <div class="gl-row-trail">
        ${c.unread ? glPill(c.unread, 'no') : ''}
        <span class="gl-row-sub mono">${fmtDate(c.last_at)}</span>
        <button class="gl-btn gl-btn-ghost" data-open="${c.user_id}">مشاهده</button>
      </div>
    </div>
  `).join('') || `<div class="empty-state"><div class="icon">${svg('empty')}</div>گفتگویی نیست</div>`;
}
...
function renderSupportRowsGlass(convs) {
  const list = $('#support-inbox-list', content());
  if (!list) return;
...
function renderTicketsGlass(tickets) {
  setContent(`
    <div class="gl-hero">
      <div><h2>تیکت‌های پشتیبانی</h2><p>${fmt(tickets.length)} تیکت در فیلتر «${TICKETS_STATUS_LABEL[ticketsStatusFilter]}»</p></div>
      <div class="gl-seg">${[['', 'همه'], ['open', 'باز'], ['answered', 'پاسخ‌داده‌شده'], ['closed', 'بسته']].map(([v, l]) => `<button class="gl-seg-btn ${v === ticketsStatusFilter ? 'active' : ''}" data-s="${v}">${l}</button>`).join('')}</div>
    </div>
    <div class="gl-list">
      ${tickets.map((t, i) => `
        <div class="gl-row bn-card-anim" style="animation-delay:${Math.min(i * 25, 220)}ms">
          ${glAvatar((t.username || '؟').trim().charAt(0), i)}
          <div class="gl-row-main">
            <span class="gl-row-title">${esc(t.subject)}</span>
            <span class="gl-row-sub">${esc(t.username || t.user_id)} · ${fmtDate(t.updated_at)}</span>
          </div>
          <div class="gl-row-trail">
            ${glPill(TICKETS_STATUS_LABEL[t.status] || t.status, t.status === 'open' ? 'pending' : t.status === 'answered' ? 'ok' : 'no')}
            <button class="gl-btn gl-btn-ghost" data-open="${t.id}">مشاهده</button>
          </div>
        </div>
      `).join('') || `<div class="empty-state"><div class="icon">${svg('empty')}</div>تیکتی نیست</div>`}
    </div>
...
function renderBroadcastGlass() {
  setContent(`
    <div class="gl-hero"><div><h2>پیام همگانی</h2><p>ارسال متنی به همه‌ی کاربران غیرمسدود ربات</p></div></div>
    <div class="gl-card" style="max-width:680px">
      <textarea class="gl-search" id="bc-text" rows="7" maxlength="4000" style="width:100%;border-radius:18px;resize:vertical" placeholder="متن پیام را بنویس..."></textarea>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px">
        <span class="mono" id="bc-count" style="font-size:12px;color:var(--text-muted)">۰ / ۴۰۰۰</span>
        <button class="gl-btn gl-btn-ok" id="bc-send" style="padding:10px 22px">ارسال به همه</button>
      </div>
      <div id="bc-result" style="margin-top:16px"></div>
    </div>
  `);
...
function renderResellersGlass(resellers, cohort) {
  const cohortHtml = cohort ? renderResellerCohortBlockGlass(cohort) : '';
  setContent(`
    <div class="gl-hero"><div><h2>نمایندگی‌ها</h2><p>${fmt(resellers.length)} نماینده ثبت‌شده</p></div></div>
    ${cohortHtml}
    <div class="gl-card-grid" style="margin-top:16px">
      ${resellers.map((r, i) => `
        <div class="gl-card bn-card-anim" style="animation-delay:${Math.min(i * 30, 260)}ms">
          <div style="display:flex;align-items:center;justify-content:space-between">
            ${glAvatar((r.username || '؟').trim().charAt(0), i)}
            ${glPill(r.is_reseller ? 'فعال' : 'غیرفعال', r.is_reseller ? 'ok' : 'no')}
          </div>
          <div class="gl-row-title">${esc(r.username || '—')}</div>
          <div class="gl-row-sub mono">#${r.telegram_id}</div>
          <div class="bw w-purple" style="border-radius:16px;padding:10px 14px;margin-top:4px">
            <span class="bw-value mono" style="font-size:18px">${fmt(r.reseller_credit_gb)} گیگ</span>
          </div>
          <button class="gl-btn gl-btn-ghost" data-credit="${r.telegram_id}" style="width:100%;margin-top:6px">تنظیم اعتبار</button>
        </div>
      `).join('') || `<div class="empty-state" style="grid-column:1/-1"><div class="icon">${svg('empty')}</div>نماینده‌ای ثبت نشده</div>`}
    </div>
  `);
...
function renderResellerCohortBlockGlass(data) {
  const c = data.churn;
  const months = data.cohorts;
  const allMonths = months.length ? months[months.length - 1].retention.map(r => r.month) : [];
  const heatRows = months.map(co => {
  ...
  return `
    ...
    <div class="gl-card" style="margin-top:14px">
      <div class="gl-row-title" style="margin-bottom:10px">کوهورت نگهداشت ماهانه</div>
      <div class="table-wrap"><table>
        <thead><tr><th>ماه کوهورت</th><th>تعداد</th>${allMonths.map((m, i) => `<th class="mono">M${i}</th>`).join('')}</tr></thead>
        <tbody>${heatRows || '<tr><td colspan="2" class="empty-state">داده‌ای نیست</td></tr>'}</tbody>
      </table></div>
...
function renderPanelsGlass(servers) {
  setContent(`
    <div class="gl-hero">
      <div><h2>سرورهای پنل VPN</h2><p>${fmt(servers.length)} سرور متصل</p></div>
      <button class="gl-btn gl-btn-ok" id="add-panel">+ پنل جدید</button>
    </div>
    <div class="gl-card-grid">
      ${servers.map((s, i) => `
        <div class="gl-card bn-card-anim" style="animation-delay:${Math.min(i * 30, 260)}ms">
          <div style="display:flex;align-items:center;gap:8px">
            <span style="width:10px;height:10px;border-radius:50%;background:${s.is_active ? 'var(--emerald)' : 'var(--rose)'};flex-shrink:0"></span>
            <span class="gl-row-title">${esc(s.name)}</span>
          </div>
          <div class="gl-row-sub">${esc(s.type_label)}</div>
          <div class="gl-row-sub mono" style="direction:ltr;text-align:left;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(s.api_url)}</div>
          <div style="display:flex;gap:8px;margin-top:6px">
...
function renderSettingsGlass(settings, rate, menuOrder) {
  setContent(`
    <div class="gl-hero"><div><h2>تنظیمات</h2><p>پیکربندی محتوا، پرداخت، کمپین و سرویس‌های ربات</p></div></div>
    <div class="gl-seg" id="settings-tabs-nav" style="margin-bottom:16px">
      ${SETTINGS_TABS.map(t => `<button type="button" class="gl-seg-btn ${t.key === settingsActiveTab ? 'active' : ''}" data-tab="${t.key}">${t.label}</button>`).join('')}
    </div>
    <div data-settings-tab="content" style="${settingsActiveTab === 'content' ? '' : 'display:none'}">
      ${menuOrderCardHtml(menuOrder)}
    </div>
    ...
    ${renderSettingsGroups(settings)}
    <div class="settings-save-bar">
      <button class="gl-btn gl-btn-ok btn-block" id="settings-save" style="width:100%;padding:12px">ذخیره تغییرات</button>
    </div>
  `);
...
function renderLogsGlass(actionsRes, res, pages) {
  setContent(`
    <div class="gl-hero"><div><h2>لاگ سیستم</h2><p>${fmt(res.total)} رکورد ثبت‌شده</p></div></div>
    <div class="gl-card" style="margin-bottom:14px">
      <div class="form-grid" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr))">
        <select class="input" id="lf-action">
          <option value="">همه‌ی عملیات</option>
          ${actionsRes.actions.map(a => `<option value="${a}" ${a === logsFilter.action ? 'selected' : ''}>${actionLabel(a)}</option>`).join('')}
        ...
        <input class="input" id="lf-id" placeholder="شناسه رکورد" value="${esc(logsFilter.record_id)}">
        <button class="gl-btn gl-btn-ok" id="lf-apply">اعمال فیلتر</button>
        <button class="gl-btn gl-btn-ghost" id="lf-clear">پاک‌کردن</button>
      </div>
    </div>
    <div class="gl-list">
      ${res.items.map((l, i) => `
        <div class="gl-row bn-card-anim" style="animation-delay:${Math.min(i * 20, 200)}ms">
          ${glAvatar('#', i)}
          <div class="gl-row-main">
            <span class="gl-row-title">${actionLabel(l.action)}</span>
            <span class="gl-row-sub">ادمین #${l.admin_id}${l.record_type ? ` · ${RECORD_TYPE_LABEL[l.record_type] || esc(l.record_type)} #${esc(l.record_id)}` : ''}${l.details ? ' · ' + esc(l.details) : ''}</span>
          </div>
          <span class="gl-row-sub mono">${fmtDate(l.created_at)}</span>
        </div>
      `).join('') || `<div class="empty-state"><div class="icon">${svg('empty')}</div>لاگی ثبت نشده</div>`}
    </div>
    <div class="pager" style="margin-top:16px">${Array.from({ length: pages }, (_, i) => i + 1).map(p => `<button class="btn btn-sm ${p === logsPage ? 'btn-primary' : ''}" data-page="${p}">${p}</button>`).join('')}</div>
...
function renderWebAdminsGlass(admins) {
  setContent(`
    <div class="gl-hero">
      <div><h2>ادمین‌های پنل</h2><p>${fmt(admins.length)} حساب ثبت‌شده</p></div>
      <button class="gl-btn gl-btn-ok" id="add-admin">+ کاربر پنل جدید</button>
    </div>
    <div class="gl-list">
      ${admins.map((a, i) => `
        <div class="gl-row bn-card-anim" style="animation-delay:${Math.min(i * 25, 220)}ms">
          ${glAvatar(a.username.trim().charAt(0).toUpperCase(), i)}
          <div class="gl-row-main">
            <span class="gl-row-title">${esc(a.username)}</span>
            <span class="gl-row-sub">${ROLE_LABEL[a.role]} · ورود: ${fmtDate(a.last_login)}</span>
          </div>
          <div class="gl-row-trail">
            ${glPill(a.is_active ? 'فعال' : 'غیرفعال', a.is_active ? 'ok' : 'no')}
            ${a.role !== 'owner' ? `
