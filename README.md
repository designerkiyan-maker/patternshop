...
> *راست‌چین — Persian RTL* — فروشگاه هوشمند الگوی خیاطی
...
| **دانلود مجدد** | فایل‌های سفارش تاییدشده از بخش «سفارش‌های من» قابل دانلود است |
...
| **پیام همگانی** | ادمین می‌تواند پیام متنی به تمام کاربرانن ارسال کند |

### ⚙️ مدیریت درون بات
| سطح | دسترسی |
|-----|-------|
| **مالک(owner)** | همه مجوزها — 12 permission کامل |
| **ادمین(admin)** | سفارشات، کاربرانن، کاتالوگ، تخفیف، تیکت، broadcast، system, settings, inventory, shipping |
| **میانی(mid)** | سفارشات، کاربرانن، تیکت، broadcast, inventory |
...
#### 📊 داشبورد
- آمار فروش (تعداد سفارش، درآمد، کاربرانن)
...
| عملیات | endpoint | مجوز |
|--------|----------|------|
| جستجوی کاربرانن | `GET /api/users?q=&status=` | users |
...
#### 📢 broadcast
- ارسال پیام متنی به تمام کاربرانن (`POST /api/broadcast`)

#### 🏪 مدیریت تجارت
| عملیات | endpoint | مجوز |
|--------|----------|------|
| سبدهای کاربرانن | `GET /api/carts` | orders |
