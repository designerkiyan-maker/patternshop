...
async def _send_inline_main_menu(u: Update, uid: int, db: Database):
    ...
    if inline_kb is not None:
        ...
        if msg:
            await msg.reply_text("📋 منو:", reply_markup=inline_kb)
...
async def make_user_handlers(db: Database, bot_token: str):
    ...
    async def cmd_start(update: Update, bot: Bot):
        ...
        await update.effective_message.reply_text(welcome, reply_markup=kb.menu_for_user(db, uid))
        await _send_inline_main_menu(update, uid, db)
        if reg_points > 0:
            try: await update.effective_message.reply_text(f"🎁 {reg_points} امتیاز خوشآمدگویی!")
            except Exception: pass
    ...
    async def _show_buy(update: Update):
        ...
        if not cats:
            await update.effective_message.reply_text("هنوز دستهبندیای اضافه نشده است.")
            return
        await update.effective_message.reply_text("📂 دستهبندیها:", reply_markup=kb.categories_kb(db, cats))
    ...
    async def _process_disc_code(update: Update):
        ...
        if not prod:
            await update.effective_message.reply_text("محصول معتبر نیست."); _clear(uid, BOT_ID); return
        ...
        if not await asyncio.to_thread(db.is_discount_code_valid, cr):
            await update.effective_message.reply_text("❌ کد تخفیف نامعتبر است.", reply_markup=kb.cancel_kb()); return
        ...
        await update.effective_message.reply_text(txt, reply_markup=kb.product_confirm_kb(pid))
    ...
    async def _process_cart_addr(update: Update):
        ...
        if not t: await update.effective_message.reply_text("لطفاً آدرس را متن ارسال کنید.", reply_markup=kb.cancel_kb()); return
        ...
        sm = await asyncio.to_thread(cart_svc.cart_summary, db, update.effective_user.id)
        await update.effective_message.reply_text("✅ آدرس ذخیره شد. تسویه را بزنید:", reply_markup=kb.cart_menu_kb(sm))
    ...
    async def _receive_receipt(update: Update, bot: Bot):
        ...
        if not oid:
            await update.effective_message.reply_text("درخواست معتبر یافت نشد."); _clear(uid, BOT_ID); return
        ...
        _clear(uid, BOT_ID)
        await update.effective_message.reply_text("✅ رسید ارسال شد. پس از تایید ادمین، فایل الگو ارسال میشود.",
                                    reply_markup=kb.menu_for_user(db, uid))
        await _send_inline_main_menu(update, uid, db)

    async def _receive_receipt_wrong(update: Update):
        await update.effective_message.reply_text("لطفاً عکس رسید را بهصورت Photo بفرستید.")
    ...
    async def _receipt_fallback(update: Update, bot: Bot):
        ...
        if order:
            try:
                ...
                await update.effective_message.reply_text("⚠️ خطایی رخ داد. دوباره تلاش کنید."); return
            await update.effective_message.reply_text("✅ رسید ارسال شد.", reply_markup=kb.menu_for_user(db, uid))
        ...
        if topup:
            try:
                ...
                await update.effective_message.reply_text("⚠️ خطایی رخ داد."); return
            await update.effective_message.reply_text("✅ شارژ کیف پول ثبت شد.", reply_markup=kb.menu_for_user(db, uid))
            await _send_inline_main_menu(update, uid, db); return
        logger.warning("Photo/doc without state for user %s", uid)
        await update.effective_message.reply_text("❌ رسید ثبت نشد. لطفاً از منوی اصلی مسیر خرید/شارژ را طی کنید.",
                                    reply_markup=kb.menu_for_user(db, uid))
        await _send_inline_main_menu(update, uid, db)
    ...
    async def _get_test_config(update: Update):
        ...
        if await asyncio.to_thread(db.count_test_files) == 0:
            await update.effective_message.reply_text("الگوی نمونه موجود نیست."); return
        ok, rem = await asyncio.to_thread(db.can_get_test, uid)
        if not ok:
            await update.effective_message.reply_text(f"شما {rem} بار گرفته‌اید. سقف: ۱ بار."); return
        samples = await asyncio.to_thread(db.get_sample_files)
        if not samples:
            await update.effective_message.reply_text("الگوی نمونه موجود نیست."); return
        f = samples[0]
        fid = f["file_id"] if isinstance(f, dict) else getattr(f, "file_id", None)
        if not fid: await update.effective_message.reply_text("خطا."); return
    ...
    async def _show_my_orders_list(target, uid, edit=False):
        ...
        if not items:
            ...
            elif target.effective_message: await target.effective_message.reply_text(t)
            return
        t = "🧵 سفارشهای من\n\nیکی را انتخاب کنید:"
        mk = kb.my_orders_menu_kb(items)
        if edit: await _edit_text(target, t, reply_markup=mk)
        elif target.effective_message: await target.effective_message.reply_text(t, reply_markup=mk)
    ...
    async def _referral_menu(update: Update, bot: Bot):
        ...
        if s.get("referral_button_enabled", "1") != "1":
            await update.effective_message.reply_text("سیستم زیرمجموعهگیری غیرفعال است."); return
    ...
    async def _wallet_menu(update: Update):
        ...
        await update.effective_message.reply_text(f"👛 کیف پول شما\n\nموجودی: {bal:,} تومان", reply_markup=kb.wallet_menu_kb())
    ...
    async def _loyalty_menu(update: Update):
        ...
        if not await asyncio.to_thread(loyalty.is_enabled, db):
            await update.effective_message.reply_text("باشگاه مشتریان غیرفعال است."); return
    ...
    async def _cb_loy_redeem(update: Update):
        ...
        except: await update.callback_query.reply_text(...)
        await update.callback_query.answer()

    async def _process_loy_redeem(update: Update):
        raw = (update.message.text or "").strip().replace(",", "")
        if not raw.isdigit() or int(raw) <= 0:
            await update.effective_message.reply_text("تعداد امتیاز (مثال: 200):"); return
        pts = int(raw); s = await asyncio.to_thread(loyalty.get_summary, db, update.effective_user.id)
        if not s["redeem_enabled"]: _clear(update.effective_user.id, BOT_ID); await update.effective_message.reply_text("تبدیل فعال نیست."); return
        if s["min_redeem"] > 0 and pts < s["min_redeem"]:
            await update.effective_message.reply_text(f"حداقل: {s['min_redeem']} امتیاز."); return
        if pts % s["redeem_points"] != 0:
            await update.effective_message.reply_text(f"مضربی از {s['redeem_points']}."); return
        if pts > s["current"]:
            await update.effective_message.reply_text("موجودی کافی نیست."); return
        toman = (pts // s["redeem_points"]) * s["redeem_toman"]
        await update.effective_message.reply_text(f"🔄 {pts} امتیاز → {toman:,} تومان", reply_markup=kb.loyalty_redeem_confirm_kb(pts))
    ...
    async def _wheel_of_fortune(update: Update, bot: Bot):
        if await asyncio.to_thread(db.get_setting, "wheel_enabled", "1") != "1":
            await update.effective_message.reply_text("گردونه شانس غیرفعال است."); return
        can, rem = await asyncio.to_thread(db.can_spin_wheel, update.effective_user.id)
        if not can:
            await update.effective_message.reply_text(f"⏳ حدود {int(rem)+1} ساعت دیگر امتحان کن."); return
        try: await bot.send_dice(update.effective_chat.id, emoji="🎰")
        except: await update.effective_message.reply_text("🎡 در حال چرخش...")
        ...
        if won and ws["prizes"]:
            ...
            await update.effective_message.reply_text(f"🎉 تبریک! 🎟 کد {pct}٪: `{code}`\n⏳ اعتبار: {ws['expiry_hours']} ساعت",
                                        parse_mode="Markdown")
        else: await update.effective_message.reply_text("😔 امروز شانس با تو نبود!")
    ...
    async def _process_topup_amount(update: Update):
        ...
        if not t.isdigit() or int(t) < 1000:
            await update.effective_message.reply_text("عدد معتبر و حداقل 1000 تومان:"); return
        ...
        await update.effective_message.reply_text(f"مبلغ {int(t):,} ت به `{card}` به نام {holder} واریز کنید و عکس رسید را بفرستید:",
    ...
    async def _receive_topup_receipt(update: Update):
        ...
        if not amt: await update.effective_message.reply_text("درخواست معتبر یافت نشد."); _clear(uid, BOT_ID); return
        fid, rt = _receipt_payload(update)
        if not fid: return
        ...
        _clear(uid, BOT_ID)
        await update.effective_message.reply_text("✅ ثبت شد. منتظر تایید باشید.", reply_markup=kb.menu_for_user(db, uid))
        await _send_inline_main_menu(update, uid, db)

    async def _topup_wrong(update: Update):
        await update.effective_message.reply_text("عکس رسید را بهصورت Photo بفرستید.")
    ...
    async def _contact_start(update: Update):
        ...
        await update.effective_message.reply_text(ct, reply_markup=kb.cancel_kb())
    ...
    async def _contact_receive(update: Update):
        ...
        await update.effective_message.reply_text("پیام شما برای پشتیبانی ارسال شد.", reply_markup=kb.menu_for_user(db, u.id))
    ...
    async def _unknown_text(update: Update):
        ...
        await update.effective_message.reply_text(w, reply_markup=kb.menu_for_user(db, uid))
        await _send_inline_main_menu(update, uid, db)
...
async def make_admin_handlers(db: Database, bot_token: str):
    ...
    async def _open_panel(update: Update):
        ...
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text("🔧 پنل مدیریت:", reply_markup=kb.admin_panel_kb(db))
    ...
    async def _process_add_cat(update: Update):
        ...
        if not name: await update.effective_message.reply_text("نام نمیتواند خالی باشد."); return
        ...
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text("✅ اضافه شد.", reply_markup=kb.admin_category_kb(db, "products"))
    ...
    async def _process_newprod_name(update: Update):
        if not _admin_only(update.effective_user.id): return
        name = (update.message.text or "").strip()
        if not name: await update.effective_message.reply_text("نام خالی نیست."); return
        d = _get_data(update.effective_user.id, BOT_ID)
        ...
        await update.effective_message.reply_text("قیمت (تومان):", reply_markup=kb.admin_back_kb("adm_products"))

    async def _process_newprod_price(update: Update):
        if not _admin_only(update.effective_user.id): return
        t = (update.message.text or "").strip()
        if not t.isdigit(): await update.effective_message.reply_text("عدد صحیح:"); return
        d = _get_data(update.effective_user.id, BOT_ID)
        ...
        await update.effective_message.reply_text("توضیحات (یا /skip):", reply_markup=kb.admin_back_kb("adm_products"))
    ...
    async def _process_newprod_desc(update: Update):
        ...
        await update.effective_message.reply_text("عکس پیشنمایش (Photo) یا /skip:", reply_markup=kb.admin_back_kb("adm_products"))

    async def _process_newprod_preview(update: Update):
        if not _admin_only(update.effective_user.id): return
        if not update.message.photo:
            await update.effective_message.reply_text("عکس را بهصورت Photo بفرستید یا /skip:"); return
        d = _get_data(update.effective_user.id, BOT_ID)
        ...
        await update.effective_message.reply_text("فایلهای الگو (PDF و مشابه) — بعد از پایان «تمام شد»:", reply_markup=kb.files_upload_done_kb())
    ...
    async def _process_newprod_files(update: Update):
        ...
        if not cfg.is_allowed_product_filename(fname):
            ...
            await update.effective_message.reply_text(f"⛔️ پسوند مجاز نیست: {fname.rsplit('.',1)[-1]}\n{allowed}")
            return
        d = _get_data(update.effective_user.id, BOT_ID)
        ...
        await update.effective_message.reply_text(f"📎 فایل {len(fs)} ثبت شد.")
    ...
    async def _process_prod_files(update: Update):
        ...
        if not cfg.is_allowed_product_filename(fname):
            allowed = "، ".join(e.upper().lstrip(".") for e in cfg.ALLOWED_PRODUCT_FILE_EXTENSIONS)
            await update.effective_message.reply_text(f"⛔️ {fname.rsplit('.',1)[-1]} مجاز نیست.\n{allowed}")
            return
        d = _get_data(update.effective_user.id, BOT_ID)
        fs = list(d.get("files") or [])
        fs.append(update.message.document.file_id)
        _set_data(update.effective_user.id, {**d, "files": fs}, BOT_ID)
        await update.effective_message.reply_text(f"📎 {len(fs)} ثبت شد.")
    ...
    async def _cb_view_order(update: Update, bot: Bot):
        ...
        if order.get("receipt_file_id"):
            ...
            await update.callback_query.reply_text(cap, reply_markup=kb.order_review_kb(oid))
        await update.callback_query.answer()
    ...
    async def _cb_view_topup(update: Update, bot: Bot):
        ...
        if tu.get("receipt_file_id"):
            ...
            await update.callback_query.reply_text(cap, reply_markup=kb.topup_review_kb(tid))
        await update.callback_query.answer()
    ...
    async def _process_disc_code(update: Update):
        ...
        if await asyncio.to_thread(db.get_discount_code, code):
            await update.effective_message.reply_text("این کد وجود دارد. نام دیگر:"); return
        _set_data(update.effective_user.id, {"disc_code": code}, BOT_ID)
        _set_state(update.effective_user.id, "adm_disc_tv", BOT_ID)
        await update.effective_message.reply_text("فرمت: `percent 20` یا `fixed 50000`", parse_mode="Markdown")
    ...
    async def _process_disc_tv(update: Update):
        ...
        if len(parts) != 2 or parts[0].lower() not in ("percent", "fixed") or not parts[1].isdigit():
            await update.effective_message.reply_text("فرمت: `percent 20` یا `fixed 50000`"); return
        ...
        await update.effective_message.reply_text("سقف استفاده (0=نامحدود):")

    async def _process_disc_mu(update: Update):
        if not _senior_admin_only(update.effective_user.id): return
        t = (update.message.text or "").strip()
        if not t.isdigit(): await update.effective_message.reply_text("عدد:"); return
        d = _get_data(update.effective_user.id, BOT_ID)
        ...
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text(f"✅ کد «{d['disc_code']}» ساخته شد.", reply_markup=kb.discount_codes_kb(await asyncio.to_thread(db.list_discount_codes)))
    ...
    async def _process_forcejoin_ch(update: Update, bot: Bot):
        ...
        try:
            ...
            await update.effective_message.reply_text("⛔️ دسترسی ندارم. ربات باید ادمین کانال باشد.", parse_mode="Markdown",
                                         reply_markup=kb.admin_back_kb("adm_forcejoin_menu")); return
        await asyncio.to_thread(db.set_setting, "force_join_channel", ch)
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text(f"✅ کانال «{chat.title}» ثبت شد.", reply_markup=kb.admin_forcejoin_menu_kb(db))
    ...
    async def _process_ref_percent(update: Update):
        if not _senior_admin_only(update.effective_user.id): return
        t = (update.message.text or "").strip()
        if not t.isdigit() or not (0 <= int(t) <= 100): await update.effective_message.reply_text("0-100:"); return
        await asyncio.to_thread(db.set_setting, "referral_percent", t)
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text(f"✅ {t}٪ تنظیم شد.", reply_markup=kb.referral_settings_kb(db))
    ...
    async def _process_ref_comm_max(update: Update):
        ...
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text(f"✅ {t if t=='0' else t+' نفر'}.", reply_markup=kb.referral_settings_kb(db))
    ...
    async def _process_ref_fc_thresh(update: Update):
        if not _senior_admin_only(update.effective_user.id): return
        t = (update.message.text or "").strip()
        if not t.isdigit() or int(t) < 1: await update.effective_message.reply_text("عدد ≥1:"); return
        await asyncio.to_thread(db.set_setting, "referral_free_config_threshold", t)
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text(f"✅ با دعوت {t} نفر الگوی رایگان.", reply_markup=kb.referral_settings_kb(db))
    ...
    async def _process_ref_ib_amt(update: Update):
        if not _senior_admin_only(update.effective_user.id): return
        t = (update.message.text or "").strip()
        if not t.isdigit() or int(t) < 0: await update.effective_message.reply_text("عدد ≥0:"); return
        await asyncio.to_thread(db.set_setting, "referral_invite_bonus_amount", t)
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text(f"✅ {int(t):,} تومان.", reply_markup=kb.referral_settings_kb(db))
    ...
    async def _process_wheel_percent(update: Update):
        ...
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text(f"✅ {t}٪.", reply_markup=kb.wheel_settings_kb(db))
    ...
    async def _process_wheel_prizes(update: Update):
        ...
        if not all(p.isdigit() and 0 < int(p) <= 100 for p in parts) or not parts:
            await update.effective_message.reply_text("مثال: 10,20,30,50"); return
        await asyncio.to_thread(db.set_wheel_prizes, [int(p) for p in parts])
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text("✅ لیست جوایز بهروزرسانی شد.", reply_markup=kb.wheel_settings_kb(db))
    ...
    async def _process_wheel_expiry(update: Update):
        if not _senior_admin_only(update.effective_user.id): return
        t = (update.message.text or "").strip()
        if not t.isdigit() or int(t) <= 0: await update.effective_message.reply_text("عدد ≥1:"); return
        await asyncio.to_thread(db.set_setting, "wheel_code_expiry_hours", t)
        _clear(update.effective_user.id, BOT_ID)
        await update.effective_message.reply_text(f"✅ {t} ساعت.", reply_markup=kb.wheel_settings_kb(db))
    ...
    async def _cmd_cancel(update: Update):
        ...
        _clear(uid, BOT_ID)
        await update.effective_message.reply_text("❌ عملیات لغو شد." if cur else "❌ عملیاتی در جریان نبود.")
        await update.effective_message.reply_text("🔧 پنل مدیریت:", reply_markup=kb.admin_panel_kb(db))
