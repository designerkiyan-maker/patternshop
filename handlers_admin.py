...
def create_admin_router(db, is_main_bot: bool = True, bot_manager=None) -> Router:
    ...
    # -------------------------------------------------------------------
    # ساخت کانفیگ شخصی: تنظیمات کلی + سرورهای پنل + قیمت‌گذاری پلکانی
    # -------------------------------------------------------------------
    ...
    async def process_panel_server_api_key(message: Message, state: FSMContext):
        ...
        await message.answer(
            f"✅ سرور «{data['name']}» اضافه شد. حالا از «تست اتصال» مطمئن شو که درست وصل می‌شود.",
            reply_markup=kb.admin_panel_kb(db, is_main_bot),
        )
    ...
    async def cb_admin_pricing_tiers(call: CallbackQuery):
        if not senior_admin_only(call.from_user.id):
            return await deny_mid(call)
        await replace_admin_view(call,
            "💰 قیمت‌گذاری پلکانی (تصاعدی، مثل مالیات):\n\n"
            "هر بخش از حجم با نرخ همان بازه حساب و جمع می‌شود. اگر حجم انتخابی کاربر از "
            "آخرین بازه هم بیشتر شود، باقیمانده با نرخ آخرین بازه حساب می‌شود.",
    ...
    async def cb_admin_pricing_tier_delete(call: CallbackQuery):
        ...
        await replace_admin_view(call, "💰 قیمت‌گذاری پلکانی:", reply_markup=kb.pricing_tiers_kb(db))
