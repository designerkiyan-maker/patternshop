...
function updateTelegramSafeArea() {
  ...
  root.style.setProperty("--telegram-safe-top", `${Number(sa.top || 0)}px`);
  root.style.setProperty("--telegram-safe-bottom", `${Number(sa.bottom || 0)}px`);
  root.style.setProperty("--telegram-safe-left", `${Number(sa.left || 0)}px`);
  root.style.setProperty("--telegram-safe-right", `${Number(sa.right || 0)}px`);

  root.style.setProperty("--telegram-content-safe-top", `${Number(csa.top || 0)}px`);
  root.style.setProperty("--telegram-content-safe-bottom", `${Number(csa.bottom || 0)}px`);
  root.style.setProperty("--telegram-content-safe-left", `${Number(csa.left || 0)}px`);
  root.style.setProperty("--telegram-content-safe-right", `${Number(csa.right || 0)}px`);
}
...
// تم هماهنگ با تلگرام (که خودش از سیستم پیروی می‌کند): اگر تم سیستم/تلگرام
// تاریک باشد data-theme="dark" ست می‌شود و با رویداد themeChanged زنده
// به‌روز می‌شود. رنگ هدر/پس‌زمینه هم هم‌رنگ تم است.
function applyColorScheme() {
  const dark = tg.colorScheme === "dark";
  document.documentElement.dataset.theme = dark ? "dark" : "clean-light";
  try {
    tg.setHeaderColor(dark ? "#0e1621" : "#ffffff");
    tg.setBackgroundColor(dark ? "#0e1621" : "#ffffff");
  } catch (e) {}
}
applyColorScheme();
try { tg.onEvent("themeChanged", applyColorScheme); } catch (e) {}
...
async function apiUpload(path, formData) {
  ...
  if (!initData) {
    console.warn("Telegram WebApp initData is empty during upload");
  }
...
async function renderStore() {
  ...
  try {
    ...
    console.log("[CATALOG DEBUG] categories =", categories);

    if (!Array.isArray(categories) || categories.length === 0) {
      content.innerHTML = `
        <div class="state-msg">
          <span class="ic">◌</span>
          در حال حاضر الگویی موجود نیست.
        </div>
      `;
      return;
    }
    ...
    const selectedCategory =
      storeCategoryView == null
        ? null
        : categories.find(
            (c) => Number(c.id) === Number(storeCategoryView)
          );
    ...
    const products = visibleCategories.flatMap(
      (c) => Array.isArray(c.products) ? c.products : []
    );

    content.innerHTML = `
      <div class="eyebrow">
        ${selectedCategory
          ? escHtml(selectedCategory.name)
          : "فروشگاه الگوها"}
      </div>
      ...
      <div class="eyebrow" style="margin-top:16px">
        ${selectedCategory ? "الگوها" : "آخرین الگوها"}
      </div>
      ...
      ${
        products.length
          ...
          : `
            <div class="state-msg">
              <span class="ic">◌</span>
              در این دسته هنوز الگویی ثبت نشده است.
            </div>
          `
      }
    `;
    ...
    if (!selectedCategory) {
      content
        .querySelectorAll(".list-row[data-cat]")
        .forEach((el) => {
          el.onclick = () => {
            storeCategoryView = Number(el.dataset.cat);
            renderStore();
          };
        });
      ...
      const back = document.getElementById("store-back-row");
  ...
  try {
    ...
    content.innerHTML = `
      ...
      <div class="card" style="position:relative">
        ...
        ${p.description
          ? `<p class="hint-text" style="white-space:pre-wrap">${escHtml(p.description)}</p>`
          : ""}

        ${available ? `
          <button class="btn" id="add-cart-btn" style="margin-top:10px">
            🛒 افزودن به سبد خرید
          </button>
        ...
        ` : `
          <div class="state-msg" style="margin-top:10px">
            ...
            این الگو در حال حاضر ناموجود است.
          </div>
        `}
      </div>
    `;
    ...
    if (addBtn) {
      addBtn.onclick = async () => {
        ...
        try {
          ...
          tg.HapticFeedback?.notificationOccurred("success");
          notify("✅ الگو به سبد خرید اضافه شد.");
          updateMiniCartBadge(result.count);

          addBtn.textContent = "✅ در سبد خرید";
          setTimeout(() => {
            if (addBtn) {
              addBtn.disabled = false;
              addBtn.textContent = "🛒 افزودن دوباره به سبد";
            }
          }, 900);
        } catch (e) {
          notify("خطا: " + e.message);
    ...
    if (buyNowBtn) {
      buyNowBtn.onclick = async () => {
        ...
        buyNowBtn.textContent = "در حال آماده‌سازی...";
...
function updateMiniCartBadge(count) {
  const btn = document.getElementById("mini-cart-floating-btn");
...
async function refreshMiniCartBadge() {
  try {
    const cart = await api("/api/cart");
    updateMiniCartBadge(cart.total_qty || 0);
    return cart;
  } catch (e) {
    return null;
  }
...
function installMiniCartButton() {
  if (document.getElementById("mini-cart-floating-btn")) return;

  const btn = document.createElement("button");
  btn.id = "mini-cart-floating-btn";
  btn.type = "button";
  btn.innerHTML = `
  ...
  btn.onclick = () => renderCart();

  document.body.appendChild(btn);

  setTimeout(refreshMiniCartBadge, 1200);
}
  ...
  try {
    ...
    if (!cart.items || cart.items.length === 0) {
      content.innerHTML = `
        ...
        <div class="card" style="text-align:center;padding:34px 18px">
          ...
          <p class="hint-text">
            محصولات موردنظر را به سبد اضافه کن و بعد همه را یکجا تسویه کن.
          </p>
        </div>
      `;

      document.getElementById("cart-back-store").onclick = renderStore;
      return;
    }
    ...
    const rows = cart.items.map((item, index) => `
      <div class="card" style="margin-bottom:10px">
        ...
        ">
          <div style="flex:1">
            <div style="font-weight:800">
              ${fmt(index + 1)}. ${escHtml(item.product_name)}
            </div>

            ${item.variant_label
              ? `<div class="hint-text">${escHtml(item.variant_label)}</div>`
              : ""}

            <div class="hint-text" style="margin-top:5px">
              ${fmt(item.unit_price)} تومان
            </div>
          </div>

          <button
            class="btn outline small cart-remove-btn"
        ...
        ">
          ...
          <b>${fmt(item.total_price)} تومان</b>
        </div>
      </div>
    `).join("");
    ...
    content.innerHTML = `
      ...
      ">
        ...
        <button class="btn outline small" id="cart-back-store"
          style="width:auto">
          ادامه خرید
        </button>
      </div>
      ...
      <div class="card">
        <div class="stat-row">
          ...
          <b>${fmt(cart.total_qty)}</b>
        </div>

        <div class="stat-row">
          <span>جمع سبد</span>
          <b>${fmt(cart.subtotal)} تومان</b>
        </div>

        <div class="stat-row">
          <span>موجودی کیف پول</span>
          <b>${fmt(cart.wallet_credit)} تومان</b>
        </div>
        ...
        <input
          ...
          style="margin-top:12px;direction:ltr;text-align:left"
        />

        <button class="btn" id="cart-checkout-btn"
          style="margin-top:10px">
          💳 تسویه ${fmt(cart.subtotal)} تومان
        </button>

        <button class="btn outline" id="cart-clear-btn"
          style="margin-top:8px">
          🗑 خالی کردن سبد
        </button>
      </div>
    ...
    document.getElementById("cart-clear-btn").onclick = async () => {
      try {
        await api("/api/cart", { method: "DELETE" });
        notify("سبد خرید خالی شد.");
        await renderCart();
      } catch (e) {
        notify("خطا: " + e.message);
      }
    ...
    document.getElementById("cart-checkout-btn").onclick = async () => {
      const btn = document.getElementById("cart-checkout-btn");
      const code = document.getElementById("cart-discount-code").value.trim();
